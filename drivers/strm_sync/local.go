...
func (d *StrmSync) writeLocal(ctx context.Context, mountPath string, objs []model.Obj) error {
	...
	return d.deleteExtra(localDir, objs)
}
...
func (d *StrmSync) deleteExtra(localDir string, objs []model.Obj) error {
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			...
			// Only empty directories are candidates. Counting one that still
			// holds scraper output would spend budget on a removal that is
	...
	pending := len(delFiles) + len(delDirs)
	if pending == 0 {
		return nil
	}
	if skip, reason := checkDeletion(d, len(objs), len(entries), pending); skip {
		d.deleteBlocked.Add(int64(pending))
		log.Warnf("[strm-sync] refused to delete %d entries (%d files, %d directories) under %s: %s; for example %v",
			pending, len(delFiles), len(delDirs), localDir, reason, deletionSample(delFiles, delDirs))
		return nil
	}
	if ok, reason := d.reserveDeleteBudget(pending); !ok {
		d.deleteBlocked.Add(int64(pending))
		log.Errorf("[strm-sync] refused to delete %d entries under %s: %s", pending, localDir, reason)
...
// than just noisy.
func deletionSample(delFiles, delDirs []string) []string {
	const max = 5
	sample := make([]string, 0, max)
	for _, group := range [][]string{delFiles, delDirs} {
